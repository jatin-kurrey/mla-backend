...
func main() {
	...
	for year, correct := range data {
		// Update ALL records for this year to be sure
		result := db.Model(&models.Milestone{}).Where("year = ?", year).Updates(map[string]interface{}{
		...
		if result.RowsAffected > 0 {
			log.Printf("Updated %d milestones for year %s\n", result.RowsAffected, year)
		}
	}
